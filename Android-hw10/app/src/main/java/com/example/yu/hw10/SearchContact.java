...
public class SearchContact extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        View view = getView();
        lsvContactList = view.findViewById(R.id.lstContactList);
        ...
        Cursor cursor = MainActivity.Contact_SQL_db.rawQuery("select * from " + MainActivity.DB_TABLE,null);

        if (cursor != null) {
            cursor.moveToFirst();
            while (!cursor.isAfterLast()) {
                String data = "Name: " + cursor.getString(1) + ", PhoneNumber: " + cursor.getString(2) + ", PhoneType: " + cursor.getString(3);
